...
usage () {
  echo "usage:  `basename $0` [simulation|analysis] --mem=<30> --cores=<10> --conf=<2000> --records=<1000> --block=<1000> --graphic"
  echo ""
  exit 1
}
...
complete="spark-submit --conf "spark.local.dir=/mongo/log/tmp" $jars $memory $cores $conf $modules $command $graphic $records $block $steps"
